...
def import_csv_files(folder_path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for filename in os.listdir(folder_path):
        if not filename.startswith('test-cpp') or not filename.endswith('.csv') or filename.endswith('l3.csv'):
            continue
    ...
    cpp_l3_dfs = []
    for filename in os.listdir(folder_path):
        if not filename.startswith('test-cpp') or not filename.endswith('l3.csv'):
            continue

        filepath = os.path.join(folder_path, filename)
        df = pd.read_csv(filepath)
        cpp_l3_dfs.append(df)

    cpp_l3_df = pd.concat(cpp_l3_dfs, ignore_index=True)
    ...
    return (cpp_df, cpp_l3_df, lua_df)
...
def plot_naive_line_cache(df: pd.DataFrame) -> None:
    naive_df = df[df['OPERATION_MODE'] == 1].groupby('M').median().reset_index()
    line_df = df[(df['OPERATION_MODE'] == 2) & (df['M'] <= 3000)].groupby('M').median().reset_index()
    ...
    with plot_theme:
        ...
        plt.title('Cache Misses of Naive vs Line')
        plt.xlabel('Size')
        plt.ylabel('Billion Cache Misses')
        plt.legend()

        output_path = os.path.join(output_folder, 'naive-line-cache.pdf')
        plt.savefig(output_path)
        plt.close()
...
def plot_line_block_cache(df: pd.DataFrame) -> None:
    line_df = df[(df['OPERATION_MODE'] == 2) & (df['M'] >= 4096)].groupby('M').median().reset_index()
    block_128_df = df[(df['OPERATION_MODE'] == 3) & (df['BLOCK_SIZE'] == 128)].groupby('M').median().reset_index()
    block_256_df = df[(df['OPERATION_MODE'] == 3) & (df['BLOCK_SIZE'] == 256)].groupby('M').median().reset_index()
    block_512_df = df[(df['OPERATION_MODE'] == 3) & (df['BLOCK_SIZE'] == 512)].groupby('M').median().reset_index()

    for cache_miss in ['L1 DCM', 'L2 DCM']:
        with plot_theme:
            plt.figure()

            plt.plot(line_df['M'], line_df[cache_miss] / 1e9, label='Line', marker='.')
            plt.plot(block_128_df['M'], block_128_df[cache_miss] / 1e9, label='Block-128', marker='.')
            plt.plot(block_256_df['M'], block_256_df[cache_miss] / 1e9, label='Block-256', marker='.')
            plt.plot(block_512_df['M'], block_512_df[cache_miss] / 1e9, label='Block-512', marker='.')

            plt.xticks(range(4096, 10241, 2048))
            plt.ylim(bottom=0)

            plt.title(cache_miss[:2] + ' Cache Misses of Line and Block per Block Size')
            plt.xlabel('Size')
            plt.ylabel('Billion Cache Misses')
            plt.legend()

            output_path = os.path.join(output_folder, 'line-block-' + cache_miss[:2].lower() + '.pdf')
            plt.savefig(output_path)
            plt.close()

def plot_line_block_l3(df: pd.DataFrame) -> None:
    line_df = df[(df['OPERATION_MODE'] == 2) & (df['M'] >= 4096)].groupby('M').median().reset_index()
    block_128_df = df[(df['OPERATION_MODE'] == 3) & (df['BLOCK_SIZE'] == 128)].groupby('M').median().reset_index()
    block_256_df = df[(df['OPERATION_MODE'] == 3) & (df['BLOCK_SIZE'] == 256)].groupby('M').median().reset_index()
    block_512_df = df[(df['OPERATION_MODE'] == 3) & (df['BLOCK_SIZE'] == 512)].groupby('M').median().reset_index()
    ...
    with plot_theme:
        ...
        plt.plot(line_df['M'], line_df['L3 TCM'] / 1e9, label='Line', marker='.')
        plt.plot(block_128_df['M'], block_128_df['L3 TCM'] / 1e9, label='Block-128', marker='.')
        plt.plot(block_256_df['M'], block_256_df['L3 TCM'] / 1e9, label='Block-256', marker='.')
        plt.plot(block_512_df['M'], block_512_df['L3 TCM'] / 1e9, label='Block-512', marker='.')

        plt.xticks(range(4096, 10241, 2048))
        plt.ylim(bottom=0)

        plt.title('L3 Cache Misses of Line and Block per Block Size')
        plt.xlabel('Size')
        plt.ylabel('Billion Cache Misses')
        plt.legend()

        output_path = os.path.join(output_folder, 'line-block-l3.pdf')
        plt.savefig(output_path)
        plt.close()
...
if __name__ == '__main__':
    ...
    (cpp_df, cpp_l3_df, lua_df) = import_csv_files(folder_path)

    plot_naive_line_time(cpp_df, lua_df)
    plot_naive_line_flops(cpp_df, lua_df)
    plot_naive_line_cache(cpp_df)
    plot_line_block_time(cpp_df)
    plot_line_block_flops(cpp_df)
    plot_line_block_cache(cpp_df)
    plot_line_block_l3(cpp_l3_df)
